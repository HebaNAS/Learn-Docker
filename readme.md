...
- We will run the container locally first to check that it is working properly. Use the following command to run the container `docker run --name ethereum -it ethereum`. This command directs docker to create a new container from the image named _ethereum_. We added the `--name` so we can assign a name to our new container for easy management later. Containers with no assigned names will have random names generated by docker. The `-it` flag is to creating an interactive bash shell in the container. 
